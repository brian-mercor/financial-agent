...
export function ChartRenderer({ chartHtml, symbol, height = '500px' }) {
  ...
  if (!chartHtml) {
    return (
      <div className="folk-card p-8 text-center">
        <TrendingUp className="h-12 w-12 text-gray-400 mx-auto mb-3" />
        <p className="text-gray-600 folk-text text-lg">No chart data available</p>
      </div>
    )
  }
  ...
  if (error) {
    return (
      <div className="folk-card bg-red-50 p-4">
        <div className="flex items-center gap-2 text-red-700">
          <AlertCircle className="h-5 w-5" />
          <span className="folk-text font-bold">Chart Error</span>
        </div>
      ...
      {isLoading && (
        <div className="absolute inset-0 bg-white/80 flex items-center justify-center z-10 rounded-lg">
          <div className="flex flex-col items-center gap-3">
            ...
            <span className="folk-text text-lg">Loading chart...</span>
          </div>
        </div>
      )}
      <div
        ref={containerRef}
        className="w-full rounded-lg overflow-hidden bg-white folk-card"
        style={{ minHeight: height }}
      />
      {symbol && (
        <div className="mt-2 folk-text text-lg text-center">
          Chart for {symbol}
        </div>
      )}
    </div>
